...
Shader"Portal/PortalShader" {
	...
	*/
	Properties{
		...
		//[HideInInspector]_AlphaTexture("Alpha Texture", 2D) = "white"{}
		...
		//The normal map distorts the Custom Renderer. _BumpScale modifies how severely this is done. To disable, set it to 0.
		_BumpScale("Custom Renderer Distort Strength", Range(-2,2)) = 0
	    ...
	    //Normal Map- also modifies distortion of the output image (see _BumpScale)
	...
	void surf(Input IN, inout SurfaceOutputStandard o)
	{
		...
		float2 sUV = IN.screenPos.xy / max(IN.screenPos.w, 0.001);

		//todo: Snippet that should assist in native depth transfer
		/*
		...
		o.Normal = UnpackNormal(tex2D(_BumpMap, TRANSFORM_TEX(IN.uv_MainTex, _BumpMap))) /** (1 - customRenderColor.a)*/;
