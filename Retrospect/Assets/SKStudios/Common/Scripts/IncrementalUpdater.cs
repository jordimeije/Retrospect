...
namespace SKStudios.Common.Utils {
    using MonoActionPair = EppyTuple<MonoBehaviour, Action>;
    using NodeObjAction = IncrementalUpdater.Node<EppyTuple<MonoBehaviour, Action>>;
    using MonoInvalidFunc = Func<IncrementalUpdater.Node<EppyTuple<MonoBehaviour, Action>>, bool>;
    using StepDict = Dictionary<UpdateStep, Dictionary<Type, IncrementalUpdater.Node<EppyTuple<MonoBehaviour, Action>>>>;
    using NodeDict = Dictionary<Type, IncrementalUpdater.Node<EppyTuple<MonoBehaviour, Action>>>;
    ...
    public class ComparisonFuncs {
        /// <summary>
        ///     Comparer for <see cref="UpdateStep" /> enums
        /// </summary>
        ...
        /// <summary>
        ///     Comparer for the <see cref="Type" /> class
        /// </summary>
...
#if SKS_DEV
                    UnityEngine.Debug.Log(string.Format("Registered {0}", node.GetData()));
#endif
                    return true;
                }
            };
...
#if SKS_DEV
            UnityEngine.Debug.Log(string.Format("Unregistered {0}", value.GetData()));
#endif
            value.Destroy();
        }
                ...
                get {
                    ...
                    if (_invalidFunc(_nextNode)) {
                        ...
                        while (!_nextNode.Equals(this))
                            if (_invalidFunc(_nextNode)) {
                                if (_nextNode.Equals(_nextNode._nextNode)) return this;
                                _nextNode = _nextNode._nextNode;
#if SKS_DEV
                                UnityEngine.Debug.Log(string.Format("Unregistered {0}", _nextNode._data));
#endif
                            }
                            else {
                                goto RETURN;
