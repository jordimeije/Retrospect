...
#pragma warning restore 0618
        ...
        //todo: VRTK broke this and it's called twice multithreaded or something resulting in two identical body colliders.
        //this fixes it.    
        private static bool I_shouldnt_have_to_do_this = false;
        private static GameObject special_baby_cache_crib = null;
        private static CapsuleCollider special_baby_cache = null;
        private static GameObject special_baby_cache_socks = null;
        private static CapsuleCollider special_baby_cache_feet = null;
        [MethodImpl(MethodImplOptions.Synchronized)]
        protected virtual void GenerateBodyCollider()
        {
            if (bodyColliderContainer == null)
            {
                if (I_shouldnt_have_to_do_this)
                {
                    bodyColliderContainer = special_baby_cache_crib;
                    bodyCollider = special_baby_cache;
                    footColliderContainer = special_baby_cache_socks;
                    footCollider = special_baby_cache_feet;
                    return;
                }

                I_shouldnt_have_to_do_this = true;
                if (customBodyColliderContainer != null)
                {
                    special_baby_cache_crib = bodyColliderContainer = InstantiateColliderContainer(customBodyColliderContainer, BODY_COLLIDER_CONTAINER_NAME, playArea);
                    special_baby_cache = bodyCollider = bodyColliderContainer.GetComponent<CapsuleCollider>();
                }
                else
                {
        ...
        protected virtual void GenerateFootCollider()
        {
            if (CalculateStepUpYOffset() > 0f)
            {
                ...
                {
                    special_baby_cache_socks = footColliderContainer = InstantiateColliderContainer(customFootColliderContainer, FOOT_COLLIDER_CONTAINER_NAME, bodyColliderContainer.transform);
                }
                else
                {
                    special_baby_cache_socks = footColliderContainer = CreateColliderContainer(FOOT_COLLIDER_CONTAINER_NAME, bodyColliderContainer.transform);
                    footColliderContainer.gameObject.layer = LayerMask.NameToLayer("Ignore Raycast");
                }

                special_baby_cache_feet = footCollider = GenerateCapsuleCollider(footColliderContainer, 0f);
            }
        }
        ...
        protected virtual void UpdateCollider()
        {
            ...
            {
                float newpresenceColliderYSize = (headset.localPosition.y - playArea.localPosition.y) - (headsetYOffset + CalculateStepUpYOffset());
